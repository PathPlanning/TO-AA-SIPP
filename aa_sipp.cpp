...
std::list<Node> AA_SIPP::findSuccessors(const Node curNode, const Map &map)
{
    ...
    for(auto m:moves)
        //if(lineofsight.checkTraversability(curNode.i + m.i,curNode.j + m.j,map))
        {
            ...
            h_value = h_values.get_value(newNode.i, newNode.j);
            int checked(0);
            if(angleNode.g <= angleNode.interval.end)
            {
                intervals = constraints->findIntervals(newNode, EAT, close, map, checked);
                sresult.total_checked += checked;
...
SearchResult AA_SIPP::startSearch(Map &map, Task &task, DynamicObstacles &obstacles, Heuristic &heuristic)
{
    ...
    sresult.pathInfo[0].avg_valid_moves = valid_moves/sresult.pathInfo[0].numberofsteps;
    return sresult;
}
...
bool AA_SIPP::findPath(unsigned int numOfCurAgent, const Map &map)
{
    ...
    openSize = 0;
    h_values.set_goal(curagent.goal_i, curagent.goal_j);
    ...
    curNode.F = h_values.get_value(curNode.i, curNode.j);
    ...
    if(goalNode.g < CN_INFINITY)
    {
        ...
        resultPath.numberofsteps = close.size();
        resultPath.nodescreated = close.size() + openSize;
