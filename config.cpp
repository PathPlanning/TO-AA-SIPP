...
bool Config::getConfig(const char* fileName)
{
    ...
    element = algorithm->FirstChildElement(CNS_TAG_USETOAASIPP);
    if (!element)
    {
        std::cout << "Error! No '"<<CNS_TAG_USETOAASIPP<<"' element found inside '"<<CNS_TAG_ALGORITHM<<"' section. Its value is set '"<<CNS_DEFAULT_USETOAASIPP<<"'."<<std::endl;
        use_to_aa_sipp = CNS_DEFAULT_USETOAASIPP;
    }
    else
    {
        value = element->GetText();
        if(value == "true" || value == "1")
            ...
            use_to_aa_sipp = false;
        else
        {
            std::cout << "Warning! Wrong '"<<CNS_TAG_USETOAASIPP<<"' value. It's set to '"<<CNS_DEFAULT_USETOAASIPP<<"'."<<std::endl;
            use_to_aa_sipp = CN_DEFAULT_USETOAASIPP;
        }
    }

    element = algorithm->FirstChildElement(CNS_TAG_USEPERFECTH);
    if (!element)
    {
        std::cout << "Error! No '"<<CNS_TAG_USEPERFECTH<<"' element found inside '"<<CNS_TAG_ALGORITHM<<"' section. Its value is set '"<<CNS_DEFAULT_USEPERFECTH<<"'."<<std::endl;
        use_perfect_h = CNS_DEFAULT_USEPERFECTH;
    }
    else
    {
        value = element->GetText();
        ...
        {
            std::cout << "Warning! Wrong '"<<CNS_TAG_USEPERFECTH<<"' value. It's set to '"<<CNS_DEFAULT_USEPERFECTH<<"'."<<std::endl;
            use_perfect_h = CN_DEFAULT_USEPERFECTH;
        }
    }
    ...
    else
    {
        ...
        stream.clear();
        stream.str("");
        if(connectedness < 2 || connectedness > 6)
        {
