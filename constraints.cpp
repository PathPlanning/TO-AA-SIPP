...
double Constraints::get_wait_constraint(const Node &curNode, double startTimeA, const section &constraint, bool &goal_collision)
{
    double radius = 1.0;
    double i0(constraint.i1), j0(constraint.j1), i1(constraint.i2), j1(constraint.j2), i2(curNode.Parent->i), j2(curNode.Parent->j);
    ...
    if(cls == 3)
    {
        interval.first = constraint.g1;
        interval.second = constraint.g1 + (sqrt(radius*radius - dist*dist) - ha);
    }
    else if(cls == 4)
    {
        interval.first = constraint.g2 - sqrt(radius*radius - dist*dist) + sqrt(db - dist*dist);
        interval.second = constraint.g2;
    }
    else if(da < radius*radius)
    {
        if(db < radius*radius)
        ...
        else
        {
            double hb = sqrt(db - dist*dist);
            interval.first = constraint.g1;
            interval.second = constraint.g2 - hb + size;
        }
    }
    else
    {
        if(db < radius*radius)
        {
            interval.first = constraint.g1 + ha - size;
            interval.second = constraint.g2;
        }
        else
        {
            interval.first = constraint.g1 + ha - size;
            interval.second = constraint.g1 + ha + size;
        }
    }
    return interval.second;
}

double Constraints::get_end_time(const Node &curNode, double startTimeA, const section &constraint, bool &goal_collision)
{
    if(constraint.i1 == constraint.i2 && constraint.j1 == constraint.j2)
        return get_wait_constraint(curNode, startTimeA, constraint, goal_collision);
    double endTimeA(startTimeA + curNode.g - curNode.Parent->g);
    ...
    double delta = constraint.g2 - constraint.g1;
    while(delta > CN_PRECISION/2.0)
    {
        if(hasCollision(curNode, startTimeA, constraint, goal_collision))
        {
            startTimeA += delta;
        }
        else
        ...
        if(startTimeA > constraint.g2 + CN_EPSILON)
        {
            startTimeA = constraint.g2;
            break;
        }
        delta /= 2.0;
    }
    if(delta < CN_PRECISION/2.0 + CN_EPSILON && hasCollision(curNode, startTimeA, constraint, goal_collision))
    {
        startTimeA = fmin(startTimeA + delta*2, constraint.g2);
    }
    return startTimeA;
}

std::vector<SafeInterval> Constraints::findIntervals(Node curNode, std::vector<double> &EAT, const std::unordered_multimap<int, Node> &close, const Map &map, int &checked)
{
    std::vector<SafeInterval> curNodeIntervals = getSafeIntervals(curNode, close, map.width);
    checked = curNodeIntervals.size();
    ...
    for(unsigned int i=0; i<curNodeIntervals.size(); i++)
    {
        ...
        while(j < sections.size())
        {
            ...
            if(hasCollision(curNode, startTimeA, sections[j], goal_collision))
            {
                ...
                cur_interval.begin += 0.01;//startTimeA + curNode.g - curNode.Parent->g;
